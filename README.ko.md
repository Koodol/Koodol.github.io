...
*   **GitHub 배포 시**: GitHub API 토큰(Contents 및 Pull requests 읽기/쓰기 권한)을 사용하여 웹 브라우저 상에서도 편집할 수 있습니다. 각 브라우저 탭은 `main`에 직접 커밋하는 대신 자체 `dashboard/<id>` 브랜치에서 작업하며, 편집 중에는 자동으로 해당 브랜치에 저장됩니다. **Commit** 버튼을 누르면 그 브랜치에 반영하고 `main`으로의 Pull Request를 열어(또는 기존 PR에 반영하여) 병합은 일반적인 PR 리뷰로 처리됩니다 - 여러 관리자가 동시에 사용해도 같은 브랜치를 두고 충돌하지 않습니다.
